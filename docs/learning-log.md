...
## 2026-08-01 / P2.5 主题运行时接线回归

- **第一性问题：** 主题菜单的选择如何穿透 CLI、渲染器和配置写入边界，而不是只在菜单里改变选中行？
- **我的初始假设：** Workbench 已经支持主题，CLI 只需调用交互终端即可自动继承当前主题。
- **最小实验：** 追踪 `/themes` 的 `theme()`、`setTheme()` 和 `renderWorkbench()` 调用链，并用交互测试模拟打开菜单、按下方向键、回车确认和状态栏渲染。
- **观察到的证据：** CLI 没有传入动态主题回调，渲染器始终回退到默认主题；CLI 也没有传入 `sessionInfo`，因此模型和工作目录不会出现在真实底部区域。接入会话级主题状态后，方向键立即改变 ANSI Palette，持久化失败时回滚；新增配置写入测试确认 Provider 配置保持不变。
- **假设哪里错了：** UI 组件已有能力不等于产品链路已接通；回调边界没有接线时，单元渲染测试会掩盖真实 CLI 集成缺口。
- **得到的可复用原则：** 对实时 UI 功能必须同时验证状态变更、渲染输入和宿主应用接线；异步持久化不应阻塞同步预览，失败则回滚当前会话状态。
- **尚未理解：** 真机不同终端对 256 色、背景色和备用屏幕的兼容差异仍需后续验收。
- **下一步：** 继续完善写入工具、Shell 的确认与审计；Anthropic Messages Provider 仍不开放。

## 2026-08-01 / 共同工具面与分层注册

- **第一性问题：** Codex、pi 和 Claude Code 都能让模型观察、修改和验证代码，KFC 应该抽象共同机制还是复制供应商工具名称？
- **我的初始假设：** 只要把现有三个只读工具注册得更多，就已经接近主流 coding agent；但没有 capability 和默认权限，工具数量增加只会扩大不可观察的风险面。
- **最小实验：** 对比三者的一手工具文档，提炼 `list/find/read/grep`、`edit/write`、`shell` 和 allowlist/approval 交集；在 KFC Registry 增加 `read/edit/execute` capability、默认开关和统一工作区工厂，加入 `find_files`、精确 `apply_patch`、不覆盖的 `write_file` 和有 timeout/output cap 的 `shell`。
- **观察到的证据：** 默认模型定义只包含四个观察工具；修改和执行工具仍注册在 Registry 但默认关闭，`/tool` 启用后才进入下一轮 Provider 请求。新增测试覆盖 glob 查找、工具开关、唯一补丁、新文件创建、覆盖拒绝和 shell 输出截断。
- **假设哪里错了：** “共同工具”不等于“所有工具默认开放”。真正共同的是能力分层、工具发现、显式权限和结构化失败；Shell 的 cwd 限制不是操作系统沙箱，必须诚实标记风险。
- **得到的可复用原则：** Provider-facing tool schema 应来自当前权限状态；安全默认值应在注册时表达，而不是依赖 UI 文案；高风险工具即使已实现，也应以显式启用作为会话边界。
- **尚未理解：** 跨平台 Shell 进程树终止、真正 OS 级沙箱、工具执行审计轨迹和交互式逐次确认仍需 P3/P4 强化。
- **下一步：** 完成真实仓库的读取—修改—测试验收，继续完善 Shell 进程树清理、工具轨迹和修改确认；Anthropic Messages Provider、MCP、Web 和 Git 专用工具仍不纳入本轮共同核心。

## 2026-08-01 / 全宽 ASCII 启动动画

- **第一性问题：** 启动界面怎样在进入 TUI 前建立 KFLOW 的视觉识别，同时不遮蔽终端尺寸、不引入图片资源或额外运行时依赖？
- **我的初始假设：** 一个居中的单行 `KFLOW CODE` 文本足以作为启动动画，但它无法体现 ASCII 字模，也没有利用 TUI 的完整宽度。
- **最小实验：** 将启动渲染拆成全宽数字雨画布、七行 ASCII 字模和逐像素显现阶段；动画帧通过依赖注入的写入函数与延迟函数测试，最终帧清空字模空白像素，只保留可读的大字，并按 TUI `rows` 将整块画布垂直居中。
- **观察到的证据：** 80×24 终端的数字雨画布严格填满 TUI 宽度，11 行动画内容位于中间；动画共 22 帧、每帧 120ms，最终输出为大号亮白 ASCII `KFLOW CODE`。关闭颜色时仍保留动画，只有无法容纳字模的窄终端降级为居中文本。
- **假设哪里错了：** “颜色关闭就应跳过动画”不是必要约束；颜色只是表现层能力，数字雨和 ASCII 字模可以用纯文本继续工作。
- **得到的可复用原则：** 启动动画应由终端列数和行数驱动而不是固定画布尺寸；最终帧必须是清晰的稳定状态，不能继续让背景粒子污染字模；窄终端应优先保证可读性而不是强行裁剪；品牌字形与背景流动应分离配色。
- **尚未理解：** 不同终端字体宽度、Unicode 方块字符和高 DPI 渲染差异仍需真机对比；当前实现使用 ASCII `#`，因此不依赖等宽 Unicode 方块的显示效果。
- **下一步：** 完成全量质量门禁、学习快照和远程提交；启动动画本轮不再扩展为图片或复杂渲染引擎。

## 2026-08-01 / Agent 工作状态动效

- **第一性问题：** 用户需要知道模型是在思考、执行工具，还是已经结束；静态 `Working` 既没有时间反馈，也无法解释 Agent 当前动作。
- **我的初始假设：** 只在状态栏替换一个固定文案就足够，但它无法证明界面仍在响应，也无法区分模型请求和 Tool Call。
- **最小实验：** 增加独立的 spinner 帧序列和可停止的 `setInterval` 生命周期；Workbench 状态新增 `thinking` / `tool` 活动，模型请求开始时启动，收到工具调用时显示工具名，收到文本时回到思考态，请求完成、取消、错误或终端退出时统一停止并清空。
- **观察到的证据：** spinner 首帧立即渲染，随后每 100ms 更新；纯状态渲染测试覆盖“模型思考中”和“执行工具”；交互测试覆盖活动状态出现、请求完成后回到 `Ready` 以及计时器停止后不再追加重绘。
- **假设哪里错了：** 动效不是单纯的视觉装饰；若定时器没有绑定请求生命周期，就会在请求结束后继续刷新、阻止进程退出并制造虚假工作状态。
- **得到的可复用原则：** 动效必须是可停止的资源，状态栏只投影核心活动状态，Tool Call 应显示具体工具名；`NO_COLOR` 只关闭 ANSI 颜色，不应关闭 spinner 本身。
- **尚未理解：** 更细粒度的 Provider 阶段（排队、首 Token、重试、流式结束）需要事件驱动 Core 后再拆分，当前先保持 Agent Loop 现有回调边界。
- **下一步：** 在真机 TTY 中观察不同字体对 Braille spinner 的显示宽度；若发现兼容性问题，再降级为 ASCII `|/-\\` 帧。

## 2026-08-01 / Tool Call 目标摘要

- **第一性问题：** 只显示 `Tool read_file` 不能回答“模型正在读哪个文件”，工具事件必须把可安全解释的目标投影到 UI。
- **我的初始假设：** 直接把完整 Tool 输入转成文本最详细，但这会把 `write_file` 内容、Patch 文本或 Shell 命令中的凭证带进状态栏和历史记录。
- **最小实验：** 按工具名称建立白名单摘要：文件类只显示路径，搜索类显示模式/查询与位置，目录类显示目录，Shell 显示截断后的命令与 cwd 并做常见内联密钥脱敏；同一摘要同时用于 spinner 和完成后的 Tool 时间线。
- **观察到的证据：** `read_file` 会显示 `文件: <path>`，`find_files` / `grep` 显示搜索目标，`shell` 显示命令与目录，写入工具不展示内容；测试覆盖各工具摘要、Shell 脱敏和交互状态栏。
- **假设哪里错了：** “信息越多越好”不适用于终端状态栏；可读性和秘密边界比复制完整参数更重要。
- **得到的可复用原则：** Tool UI 应使用按名称白名单的摘要投影，而不是通用 JSON 序列化；活动态和完成态应复用同一个摘要，避免用户看到前后不一致的目标。
- **尚未理解：** Tool Result 的结构化结果摘要（例如读了多少行、搜索命中多少条）还需要 Agent Loop 暴露工具完成事件后再补充。
- **下一步：** 在不泄露内容的前提下，为 Tool Result 增加完成数量摘要，并继续保持路径、命令和错误信息的截断边界。

## 2026-08-01 / Tool Result 完成摘要

- **第一性问题：** “正在执行什么”与“执行结果如何”是两个不同事实；只有 Tool Call 没有 Tool Result，时间线仍然无法证明工具完成、失败或超时。
- **我的初始假设：** TUI 可以在工具开始后直接猜测结果，或把原始 JSON 直接输出；前者不可靠，后者会泄露文件内容和 Shell 输出。
- **最小实验：** Agent Loop 增加 `onToolResult` 事件，携带原子 Tool Call、结构化结果和耗时；TUI 使用按工具白名单的摘要器，把读行数、搜索命中数、写入字节、Shell exit code、超时、截断和稳定错误码投影到时间线。
- **观察到的证据：** `read_file` 显示 `读取 N 行`，`grep` 显示 `命中 N 条`，`apply_patch` 显示替换数和写入字节，Shell 显示退出码与截断状态；原始结果内容不会进入界面。Agent、摘要器、WorkBench 和完整交互测试均通过。
- **假设哪里错了：** “结果摘要越详细越好”仍然错误；摘要必须是结构化白名单，否则 Tool Result 会绕过已有的终端输出和秘密边界。
- **得到的可复用原则：** Agent Core 负责发出事实事件，UI 只负责安全投影；耗时应在执行边界内测量；错误结果也必须形成可观察但不泄露原因细节的完成事件。
- **尚未理解：** 多个并行 Tool Call、Tool Result 流式输出和跨会话持久化轨迹仍需事件模型与会话存储支持。
- **下一步：** 完成一次真实“读取—修改—测试—解释”验收，并在写操作前加入逐次确认；Anthropic Messages Provider、MCP 和 Web 工具继续不纳入当前主线。

## 2026-08-01 / TUI Markdown 投影与逐次工具授权
...
- **尚未理解：** 完整 CommonMark、Unicode 宽度、终端复制选区和跨平台 Shell 进程树仍需真机验收；批量授权、allowlist 和持久化审计留给后续 Permission Policy/P4。
- **下一步：** 在真实工作区完成一次“读取—修改—测试—解释”闭环，并补充 `git diff` 检查与失败恢复说明；Anthropic Messages Provider、MCP、Web 和 Git 专用工具继续不纳入当前主线。

## 2026-08-01 / P3.1 Git 变更检查与失败恢复说明

- **第一性问题：** 修改和测试之间缺少一个可验证的事实层时，系统如何知道哪些变化来自本轮、哪些本来就存在？
- **我的初始假设：** 调用 `git diff` 就足够；但它默认不显示未跟踪文件，也无法区分会话开始前的脏状态，失败时自动回滚更危险。
- **最小实验：** 新增只读 `git_diff` 工具，在工具创建时记录会话基线；执行时汇总 status、numstat 和未跟踪文件；编辑结果标记 `changed`，Shell 标记 `unknown`，交互失败时投影已完成/失败工具和恢复说明。
- **观察到的证据：** 临时 Git 仓库完成 `read_file → apply_patch → git_diff → shell`，得到 1 个文件、`+1/-1`、测试 `exitCode=0`；既有未跟踪文件被标记为 `preexisting`，没有泄露完整 patch。
- **假设哪里错了：** “恢复”不是把仓库强行恢复到 HEAD；用户原有脏状态可能比 Agent 更早，安全恢复的最小动作是再次观察、人工审查和明确下一步。
- **得到的可复用原则：** 可恢复性首先来自可归因的事实记录，其次才是人工或未来的回滚机制；Shell 这类通用执行器不能假装知道是否改了文件。
- **尚未理解：** 基线跨会话持久化、并行工具调用的变更归因和 Git rename/binary 的更细粒度摘要仍待事件与会话层支持。
- **下一步：** 进入 P4 事件驱动 Core 前，先收集实际 TUI 使用反馈；继续不加入 Anthropic Messages Provider、MCP、Web 或自动 Git 写操作。

## 2026-08-01 / 生产 Agent 取消固定步数上限

- **第一性问题：** 长任务 Agent 的“完成条件”应该由一个固定数字决定，还是由模型自然结束、用户中断和系统错误共同决定？
- **我的初始假设：** 默认 8 步可以防止循环；真实长任务证明它会在任务尚未完成时粗暴截断，且用户只能修改一个环境变量重新开始。
- **最小实验：** 将 `AgentRunRequest.maxSteps` 扩展为 `number | "unlimited"`，CLI/TUI 生产路径传入 `"unlimited"`，保留数字预算供确定性测试；用 70 次工具调用的脚本验证不会被原 64 步边界截断。
- **观察到的证据：** unlimited 测试完成 70 次工具调用后进入第 71 轮最终文本；现有取消、Provider 错误、工具错误和权限边界仍然保留。
- **假设哪里错了：** “有上限才安全”过于粗糙；真正安全的是可中断、可观察、可归因。固定步数只是隐藏循环问题，不是完成判断。
- **得到的可复用原则：** 生产 Agent 不应把教学用预算伪装成用户任务的完成条件；应把终止控制放在用户中断、Provider/上下文边界、工具错误和后续重复行为检测上。
- **尚未理解：** 无限模式下的重复 Tool Call、Token 消耗、上下文增长和“继续/停止”交互仍需事件与会话层支持。
- **下一步：** P4 增加 Agent 事件、重复调用检测和实时成本/上下文观测；暂不重新引入固定步数配置。

## 2026-08-01 / 无限 Agent Loop 的无进展保护与运行观测

- **第一性问题：** 取消固定步数后，系统如何避免模型在同一个工具动作上无限消耗，同时让用户知道长任务实际做了什么？
- **我的初始假设：** 只要允许 Esc/Ctrl+C，中断责任就可以完全交给用户；但这把模型重复调用造成的 Token 浪费和“无进展”判断全部推给了用户。
- **最小实验：** 为每个已完成工具轮次生成不含 Tool Call ID 的稳定指纹；连续三轮请求和结果相同则抛出 `AGENT_REPEATED_TOOL_CALL`。同时在 Agent 结果、CLI stderr 和 TUI `/status` 接入模型轮次、工具调用、失败调用、耗时、首字延迟、峰值输入 Token 和 usage。
- **观察到的证据：** 70 次不同输入的工具调用仍能在 unlimited 模式完成；三轮相同 `lookup({})` 且结果一致时在第三轮停止；CLI 和 TUI 都能显示实际运行指标，未配置价格表时明确显示成本未知。
- **假设哪里错了：** “没有固定上限”不等于“没有终止策略”；长任务需要的是基于进展的安全边界，而不是一个与任务内容无关的数字。
- **得到的可复用原则：** 无限循环必须与可观察性绑定；先记录事实，再用重复行为判断无进展；成本字段没有价格依据时应保持未知，不伪造估算。
- **尚未理解：** 当前指标还不是领域事件，错误中断时也没有把部分运行指标作为可恢复轨迹持久化。
- **下一步：** 进入 P4，设计 Agent/Tool/Authorization/Completion 统一事件和 JSONL 会话持久化；暂不加入 Anthropic Messages Provider、MCP、Web 或 Subagents。

## 2026-08-01 / JSONL 会话日志与崩溃诊断

- **第一性问题：** 长任务在 Provider 中断或终端关闭后，系统如何保留足够事实来诊断，而不冒险自动回滚工作区？
- **我的初始假设：** 保存最终消息就足够；但如果进程在工具调用中途退出，只有最终消息无法说明哪一步已经执行。
- **最小实验：** 新增 `SessionStore`，串行追加 `0600` JSONL 事件；TUI 接入会话开始、轮次、工具调用/结果、完成/失败、清除和退出事件；读取时用 Zod 逐行校验并返回损坏行号。
- **观察到的证据：** 一个完整轮次可以按 `session.started → turn.started → tool.call → tool.result → turn.completed → session.ended` 重放；无效 JSON 和错误 schema 只产生诊断 issue，不丢弃其他合法事件；退出前 flush 保证最后事件有机会落盘。
- **假设哪里错了：** “恢复”不等于立即重放或 reset；第一步应是保存可归因事实，再由用户审查是否继续。
- **得到的可复用原则：** 持久化边界应围绕状态转换而不是 UI 文本；JSONL 适合追加和部分损坏诊断；文件权限是会话隐私边界的一部分。
- **尚未理解：** 会话日志保留周期、跨版本 schema migration、恢复时如何避免重复执行工具仍未解决。
- **下一步：** 完成 P4 的统一领域事件，再设计只读会话诊断和用户确认后的 resume；Anthropic Messages Provider、MCP、Web 和 Subagents 继续不纳入主线。
