...
- **最小实验：** 将启动渲染拆成全宽数字雨画布、七行 ASCII 字模和逐像素显现阶段；动画帧通过依赖注入的写入函数与延迟函数测试，最终帧清空字模空白像素，只保留可读的大字。
- **观察到的证据：** 80 列终端的每一行都严格填满 TUI 宽度；动画共 22 帧、每帧 120ms，最终输出为大号 ASCII `KFLOW CODE`。关闭颜色时仍保留动画，只有无法容纳字模的窄终端降级为居中文本。
- **假设哪里错了：** “颜色关闭就应跳过动画”不是必要约束；颜色只是表现层能力，数字雨和 ASCII 字模可以用纯文本继续工作。
- **得到的可复用原则：** 启动动画应由终端列数驱动而不是固定画布宽度；最终帧必须是清晰的稳定状态，不能继续让背景粒子污染字模；窄终端应优先保证可读性而不是强行裁剪。
