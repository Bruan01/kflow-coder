...
describe("KFlow startup animation", () => {
  ...
  it("fills the TUI width with digital rain and reveals a large ASCII logo", () => {
    const frames = createStartupFrames({ columns: 80, rows: 24, color: true });
    ...
    expect(firstLines.slice(0, 6).every((line) => line === "")).toBe(true);
    expect(firstLines.slice(6, 17).every((line) => line.length === 80)).toBe(
      true,
    );
    expect(firstLines.slice(17).every((line) => line === "")).toBe(true);
    expect(firstFrame).toMatch(/[0-9]/);
    expect(finalFrame).toContain("#####");
    expect(finalFrame).toContain("#   #");
    expect(frames.at(-1)).toContain("\u001b[1;37m#");
  });

  it("keeps the large ASCII animation when color is disabled", () => {
    const frames = createStartupFrames({ columns: 80, rows: 24, color: false });
  ...
  it("writes bounded frames through injected timing without network or model work", async () => {
    ...
    expect(write.mock.calls[0]?.[0]).toContain("\u001b[2J\u001b[H\n\n\n\n\n\n");
